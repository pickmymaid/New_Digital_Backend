...
passport.use(new AppleStrategy({
    ...
    // In the K8s deployment this path is populated by an initContainer that
    // decodes the APPLE_AUTH_KEY_P8_BASE64 secret into a shared emptyDir
    // volume mounted at /secrets (kept separate from /app so it can't
    // collide with the app source copied into the image). See
    // k8s/backend2.yaml.
    privateKeyLocation: process.env.APPLE_AUTH_KEY_PATH || "/secrets/AuthKey.p8",
